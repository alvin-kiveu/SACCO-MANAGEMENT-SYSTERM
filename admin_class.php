...
class Action
{
	...
	function save_loan()
	{
		...
		//CHECK IF USER HAS AN EXITING LOAN 
		$chk = $this->db->query("SELECT * FROM loan_list where borrower_id = $borrower_id and status = 2 ");
		if ($chk->num_rows > 0) {
			return 2;
			exit;
		} else {
			...
			if (isset($status)) {
				...
				if ($status == 2) {
					...
					$this->db->query("UPDATE loan_schedules set status = 3 where loan_id = $id ");
				} else {
	...
	function save_payment()
	{
		...
		$chaecksql = $this->db->query("SELECT * FROM loan_list where id = $loan_id");
		if ($chaecksql->num_rows > 0) {
			$check = $chaecksql->fetch_array();
			$loanamount = $check['amount'];
			$planid = $check['plan_id'];
			$plan = $this->db->query("SELECT * FROM loan_plan where id = $planid")->fetch_array();
			$planinterest = $plan['interest_percentage'];
			$planinterest = $planinterest / 100;
			$totalinterest = $loanamount * $planinterest;
			...
			$payments = $this->db->query("SELECT * FROM payments where loan_id = $loan_id");
			$amountpaid = 0;
			while ($row = $payments->fetch_assoc()) {
				$amountpaid += $row['amount'];
			}
		  //CONFIRM IF PAYMENT IS EQUAL TO LOAN AMOUNT
			if ($amountpaid >= $totalamount) {
				$this->db->query("UPDATE loan_list set status = 3 where id = $loan_id");
			}
		}else{
			$save = $this->db->query("UPDATE payments set " . $data . " where id = " . $id);
		}
